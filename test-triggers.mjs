...
async function run() {
  console.log('Trying to fetch triggers via RPC (if an admin function exists)...');
  const { data, error } = await db.rpc('get_triggers');
  if (error) {
    console.error('Cannot fetch triggers via RPC:', error.message);
  } else {
    console.log(data);
  }
}
