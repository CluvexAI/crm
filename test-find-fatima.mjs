...
async function run() {
  const { data, error } = await db.from('crm_leads').select('id, contact_name, owner_phone, email, created_at, updated_at');
  const fatima = data.filter(l => l.contact_name && l.contact_name.includes('Fatima'));
  console.log('Results for Fatima:');
