...
async function run() {
  // Since we cannot run raw sql, I will check the entire DB using postgrest if possible,
  // but we don't have access to pg_trigger via anon key either.
  // Wait! I can look at the backend logs! 
  // No, I can't.
  
  // Let me just look closely at mapLeadToDB again!
  console.log("I need to find the bug!");
}
run();
