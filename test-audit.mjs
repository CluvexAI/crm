...
async function run() {
  // We can't query information_schema directly with anon key, but we can try to fetch from audit_logs
  const { data, error } = await insforge.database.from('audit_logs').select('*').limit(5);
  console.log('audit_logs:', error || data);
}
